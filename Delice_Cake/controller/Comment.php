...
class CommentController {
     public function add() {
        if (isset($_SESSION['id_nguoidung'], $_POST['id_banh'], $_POST['noi_dung'], $_POST['xephang'])) {
            ...
            $sql_check = "SELECT COUNT(*) FROM tbl_binhluan_tintuc WHERE id_tintuc = ? AND id_nguoidung = ?";
            ...
            $sql = "INSERT INTO tbl_binhluan_tintuc (ma_binhluan, id_nguoidung, id_tintuc, noi_dung, xephang) VALUES (?, ?, ?, ?, ?)";
    ...
    public function update() {
        if (isset($_SESSION['id_nguoidung'], $_POST['id_bl'], $_POST['noi_dung'], $_POST['xephang'])) {
            ...
            $sql = "UPDATE tbl_binhluan_tintuc SET noi_dung = ?, xephang = ? WHERE id_bl = ? AND id_nguoidung = ?";
            $db = new connect();
            $stmt = $db->prepare($sql);
            if ($stmt === false) {
                header('Content-Type: text/plain');
